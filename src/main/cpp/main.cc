...
class MyFilter : public ROCKSDB_NAMESPACE::CompactionFilter {
 public:
  bool Filter(int level, const ROCKSDB_NAMESPACE::Slice& key,
    ...
    fprintf(stderr, "Filter(%s) Value:(%s) value_changed(%d)\n", key.ToString().c_str(),existing_value.ToString().c_str(),*value_changed);
    ++count_;
    assert(*value_changed == false);
    if (existing_value.compare("bad") == 0){
        fprintf(stderr, "Filter key(%s) Value:(%s) value_changed(%d)\n", key.ToString().c_str(),existing_value.ToString().c_str(),*value_changed);
        return true;
    }
    return false;
  }
...
//   options.merge_operator.reset(new MyMerge);
  ...
  db->Put(wopts, "0", "bad");  // This is filtered out
  db->Put(wopts, "1", "data1");
  db->Put(wopts, "1", "bad");
  db->Put(wopts, "1", "data2");
  db->Put(wopts, "1", "bad");
  db->Put(wopts, "3", "data3");
